...
export type CustomerListData = { name: string; email: string; location: string; orders: number; date: string; status: number };
// table data
function createData(name: string, email: string, location: string, orders: number, date: string, status: number) {
    return { name, email, location, orders, date, status };
}

const rowsInitial: CustomerListData[] = [
...
function stableSort(array: CustomerListData[], comparator: (a: CustomerListData, b: CustomerListData) => number) {
    const stabilizedThis = array.map((el: CustomerListData, index: number) => [el, index]);
    stabilizedThis.sort((a, b) => {
        const order = comparator(a[0] as CustomerListData, b[0] as CustomerListData);
...
export interface CustomerListEnhancedTableHeadProps extends EnhancedTableHeadProps {
    theme: Theme;
    selected: string[];
}

function EnhancedTableHead(props: CustomerListEnhancedTableHeadProps) {
    const { onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort, selected } = props;
...
const CustomerList = () => {
    ...
    const [rows, setRows] = React.useState<CustomerListData[]>(rowsInitial);
