...
export type OrderData = { id: string; name: string; company: string; type: string; qty: number; date: string; status: number };

// table data
function createData(id: string, name: string, company: string, type: string, qty: number, date: string, status: number) {
    return { id, name, company, type, qty, date, status };
}

const rowsInitial: OrderData[] = [
...
function stableSort(array: OrderData[], comparator: (a: OrderData, b: OrderData) => number) {
    const stabilizedThis = array.map((el: OrderData, index: number) => [el, index]);
    stabilizedThis.sort((a, b) => {
        const order = comparator(a[0] as OrderData, b[0] as OrderData);
...
function EnhancedTableHead(props: OrderListEnhancedTableHeadProps) {
...
const OrderList = () => {
    ...
    const [rows, setRows] = React.useState<OrderData[]>(rowsInitial);
